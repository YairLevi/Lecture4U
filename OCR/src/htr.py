...
def infer(model: Model, fn_img: Path, out_file_name, image_name, text) -> None:
    ...
    # ========================================================================================
    chars = ['"', ' ', '.', ':', ',', ';', '']
    if recognized[0] not in chars:
        '''
        f = open(out_file_name, "a+")
        f.write(f'image name: "{image_name}"\n')
        f.write(f'Recognized: "{recognized[0]}"\n')
        f.write(f'Probability: {probability[0]}\n')
        f.write("===========================================\n")
        f.close()
        '''
        f = open(text, "a+")
...
def main(image_name, out_file_name, text):
    ...
    # infer text on test image
        ...
        infer(model, args.img_file, out_file_name, image_name, text)
