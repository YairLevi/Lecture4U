...
def handle_htr(text, words_split_const):
    lines, l_idx = glob.glob(LINES_ROI_PNG), 0               # 18: 58 -> 0.48, *59 -> 0.74, 60 -> 0.48, 61 -> 0.48
    for img in lines:                                        # 17: 58 -> 0.43, *59 -> 0.43, 60 -> 0.43, 61 -> 0.49
        split_image_text(img, WORDS_ROI, words_split_const)  # 14: 58 -> 0.48, *59 -> 0.50, 60 -> 0.48, 61 -> 0.53
        current_line = glob.glob(WORDS_ROI_PNG)              # 16: 58 -> 0.74, *59 -> 0.74, 60 -> 0.74, 61 -> 0.54
...
def transcript(to_eval, text, words_split_const, to_detect):
    ...
    if to_detect:
        cv2.imwrite('init_image.png', split_image_text("..\\image examples\\" + filename, LINES_ROI, words_split_const))
        temp_folders()
    else:
